...
export function getFeedbackStore() {
	return {
		...
		init(sid: string, planTitle: string, planVersion: number, existing?: FeedbackPayload | null) {
			sessionId = sid;
			if (existing) {
		...
		addComment(section: string, quote: string, comment: string, phaseId?: string) {
		...
		deleteComment(id: string) {
			if (!currentFeedback) return;
			currentFeedback.comments = currentFeedback.comments.filter((c) => c.id !== id);
			currentFeedback.updatedAt = new Date().toISOString();
			debouncedSave();
		},

		setPhaseStatus(phaseId: string, status: 'pending' | 'approved' | 'needs-work', note?: string) {
			if (!currentFeedback) return;
